...
type IUserReportService interface {
	GenerateExcel(users *[]models.User) ([]byte, error)
}
...
func (s *userReportService) GenerateExcel(users *[]models.User) ([]byte, error) {
	f := excelize.NewFile()
	...
	if err := f.Write(buf); err != nil {
		return nil, responses.NewErrorRes(fiber.StatusInternalServerError, []string{"無法寫入 Excel 檔案"})
	}
